Ragecoin Core version *0.15.1* is now available from:

  <https://download.ragecoin.org/ragecoin-0.15.1/>
...
A number of changes to the way Ragecoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 rages per kB (0.00001 RAGE). This is to prepare for dropping the minimum transaction fee to 0.1 rages per kB (0.0001 RAGE) in the next release.
